...
class RW_Taxonomy_Meta
{
    ...
    function __construct($meta)
    {
        ...
        add_action('load-edit-tags.php', array(
            $this,
            'load_edit_page'
        ));
    }
    ...
    function load_edit_page()
    {
        ...
        if ('edit-tags' != $screen->base || empty($_GET['action']) || 'edit' != $_GET['action'] || !in_array($screen->taxonomy, $this->_taxonomies)) {
            return;
        }
    ...
    function add()
    {
        ...
        foreach (get_taxonomies() as $tax_name) {
            if (in_array($tax_name, $this->_taxonomies)) {
                ...
                //add_action($tax_name . '_add_form', array( $this, 'show' ), 9, 2);
            }
        }
    }
...
%s <a class="rwtm-delete-file" href="#">%s</a>
<input type="hidden" name="%s[]" value="%s">
</li>', wp_get_attachment_image($att), __('Delete', 'rwtm'), $field['id'], $att);
        }
        echo '</ul>';
    ...
    function normalize()
    {
        ...
        foreach ($this->_fields as &$field) {
            ...
            $style     = in_array($field['type'], array(
                'text',
                'textarea'
            )) ? 'width: 95%' : '';
            $optgroups = false;
